...
def raw_to_staging():
  ...
  df_jan = pl.read_parquet('./data/raw/yellow_tripdata_2023-01.parquet')
  df_zones = pl.read_csv('./data/raw/zone_lookup.csv')
  ...
  df_jan = df_jan.with_columns(
    pl.when(pl.col('payment_type') == 1).then(pl.lit("Credit card")).when(pl.col('payment_type') == 2).then(pl.lit("Cash"))\
      .when(pl.col("payment_type") == 3).then(pl.lit("No charge")).when(pl.col("payment_type") == 4).then(pl.lit("Dispute"))\
  ...
  df_zones = df_zones.rename({"Borough": "borough_name", "LocationID": "location_id", "Zone": "zone"})

  df_joined.write_parquet('./data/staging/yellow_tripdata_2023-01.parquet')
  df_zones.write_parquet('./data/staging/zone_data.parquet')